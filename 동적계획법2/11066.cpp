...
// 부분합을 더해주는 이유는 start 부터 mid까지의 임시파일을 구하고 (dp[start][mid] 최소값)
// mid+1부터 end 까지의 임시파일을 구한 다음 (dp[mid+1][end] 최소값)
// 이 두개의 임시파일을 최종적으로 더해주는 값이 start부터 end까지의 모든 파일을 합하는 비용과 같기 때문이다
